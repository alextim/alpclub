...
### Форма "Контакты"
То что посетители сайта послали через контактную форму можно посмотреть через адмиин-панель.

Пунккт меню "AT Contact Form".

- "Messages" - список посланных сообщений в хронологическом порядке.
- "Addrees Book" - записная книжка с e-mail и именем посетителя. Так же хранится время и IP, откуда отправлялась последняя форма.
