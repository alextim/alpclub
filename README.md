...
- Для списков пользуйтесь тэгами `ul`, `ol` и `li`.
  ...
  :bulb: Не создавайте неуправляемые ламерские списки с помощью тире или цифр.
...
```
:bulb: <i>При использовании полного пути после смены протокола `https` на `http` или переходе на новый домен ссылка будет утеряна.</i>
...
:bulb: <i>Размер файла картинки очень влияет на производительность сайта. Будьте внимательны, следите за размером. 
Сжимайте!</i>
...
:bulb: <i>Чтобы пост, выбранный в элементе "ACO: Special post" не дублировался в элементе "ACO: Latest news" перейдите в последнем по ссылке "Редактировать". Затем в комбобоксе "Exclude Post:" выберите пост, который хотите исключить.
Для сохранения изменений нажмитек кнопку "Выполнено".</i>


## <a name="to-admin"></a>2. Администратору
### <a name="to-important"></a>**![#f03c15](https://placehold.it/15/f03c15/000000?text=+) Важное!!!**
- Не забывайте регулярно делать бекап файлов сайта и самой базы данных. Имейте несколько бэкапов, сохраняйте бэкап по датам.
...
:bulb: Если ни один чек-бокс не выбран, то форма "Контакты" не показывается.
